...
class LstmQAHead(nn.Module):
    def __init__(self, input_size):
        ...
        self.pooler = nn.Linear(1536, 1) # nn.AdaptiveAvgPool1d(-1)
...
class CnnQAHead(nn.Module):
    def __init__(self, input_size):
        super().__init__()
        self.conv_1 = nn.Conv1d(in_channels=input_size, out_channels=1, kernel_size=1, padding=0)
        self.conv_3 = nn.Conv1d(in_channels=input_size, out_channels=1, kernel_size=3, padding=1)
        self.conv_5 = nn.Conv1d(in_channels=input_size, out_channels=1, kernel_size=5, padding=2)
